...
//
$_CONFIG['secondary_titles'] = array("Bem vindo à minha pasta pessoal de arquivos<br/>Cuidado com o que vai baixar.");
...
//
$_CONFIG['hidden_files'] = array(".ftpquota", "index.php", "index.php~", ".htaccess", ".htpasswd", ".log-de-acesso.txt");
...
//
$_CONFIG['users'] = array(array("", "", "admin"));
...
//
$_CONFIG['log_file'] = ".log-de-acesso.txt";
		...
		if($open_dir = @opendir($this->location->getFullPath()))
		{
			...
			while ($object = readdir($open_dir))
			{
				if($object != "." && $object != "..")
				{
					...
					{
						if(!in_array($object, EncodeExplorer::getConfig('hidden_dirs')) || (GateKeeper::isUserLoggedIn() == true && GateKeeper::getUserStatus() == "admin"))
